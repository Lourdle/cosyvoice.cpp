...
cosyvoice_llm_kv_cache::cosyvoice_llm_kv_cache(
	...
	ggml_type kv_type,
	bool fattn) :
	cur_len(0), layers(layers), fattn(fattn), offloaded_cache(nullptr),
	num_attention_heads(num_attention_heads), kv_cache_layers(new kv_cache_layer[layers]), ctx(nullptr)
{
